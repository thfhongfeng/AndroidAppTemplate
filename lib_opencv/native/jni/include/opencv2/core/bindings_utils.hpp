...
//! @{
        ...
        CV_EXPORTS_W String
        dumpInputArray(InputArray
        argument);

        CV_EXPORTS_W String
        dumpInputArrayOfArrays(InputArrayOfArrays
        argument);

        CV_EXPORTS_W String
        dumpInputOutputArray(InputOutputArray
        argument);

        CV_EXPORTS_W String
        dumpInputOutputArrayOfArrays(InputOutputArrayOfArrays
        argument);

        CV_WRAP static inline
                String

        dumpBool(bool argument) {
...
dumpCString(const char *argument) {
...
dumpString(const String &argument) {
...
testOverloadResolution(const Rect &rect) {
...
dumpTermCriteria(const TermCriteria &argument) {
...
dumpRotatedRect(const RotatedRect &argument) {
...
testRotatedRect(float x, float y, float w, float h, float angle) {
...
dumpRange(const Range &argument) {
...
testReservedKeywordConversion(int positional_argument, int lambda = 2, int from = 3) {
    return format("arg=%d, lambda=%d, from=%d", positional_argument, lambda, from);
}

CV_EXPORTS_W String

dumpVectorOfInt(const std::vector<int> &vec);

CV_EXPORTS_W String

dumpVectorOfDouble(const std::vector<double> &vec);

CV_EXPORTS_W String

dumpVectorOfRect(const std::vector <Rect> &vec);
...
dumpVec2i(const cv::Vec2i value = cv::Vec2i(42, 24)) {
...
namespace nested {
    ...
    class CV_EXPORTS_W CV_WRAP_AS(ExportClassName)
