 ...
 */
                ...
                GSource(std::shared_ptr<IDataProvider>
                source,
                const CfgParams &cfg_params = CfgParams{}
                );
                ...
                private:
                explicit GSource(std::unique_ptr<Priv>
                && impl);
        ...
        inline make_onevpl_src(Args
        &&... args) {
        return

        make_src<onevpl::GSource> (std::forward<Args>(args)
