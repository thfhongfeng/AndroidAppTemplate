...
namespace cv {
    namespace detail {
        ...
        template<template<class> class, typename, typename...>
        struct all_satisfy;
...
template<std::size_t S0, std::size_t... SS>
struct max_of_t {
    static constexpr const std::size_t
    rest = max_of_t<SS...>::value;
    static constexpr const std::size_t
    value = rest > S0 ? rest : S0;
};
...
struct contains<T,
        std::tuple < Types...>> : std::integral_constant<bool, contains<T, Types...>::value> {
};
