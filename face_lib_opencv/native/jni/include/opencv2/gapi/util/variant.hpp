...
namespace cv {
    namespace util {
        ...
        struct type_list_index {
            static const constexpr std::size_t
            value = detail::type_list_index_helper<0, Target, Types...>::value;
        };
        ...
        class variant {
            ...
            S = cv::detail::max_of_t<sizeof(Ts)...>::value;
            static constexpr const std::size_t
            A = cv::detail::max_of_t<alignof(Ts)...>::value;
            ...
            static constexpr std::array<cnvrt_assgn_t, sizeof...(Ts)>
            ...
            template<bool is_lvalue_arg>
            static constexpr std::array<cnvrt_ctor_t, sizeof...(Ts)>
        ...
        T *get_if(util::variant<Types...> *v)

        noexcept;

        template<typename T, typename... Types>
        const T *get_if(const util::variant<Types...> *v)
        ...
        template<std::size_t Index, typename... Types>
        const typename util::type_list_element<Index, Types...>::type &
        get(const util::variant<Types...> &v);

        template<typename T, typename... Types>
        bool holds_alternative(const util::variant<Types...> &v)
        ...
        noexcept {
        typedef typename std::tuple_element<0, std::tuple < Ts...> >
        ::type TFirst;
        new (memory)
...
variant<Ts...>::variant(T &&t)
        : m_index(util::type_list_index < util::decay_t < T > , Ts...
...
cnvrt_ctors<is_lvalue_arg>()[m_index]

)(memory, const_cast<util::decay_t <T> *>(&t));
}
...
template<typename... Ts>
variant<Ts...> &variant<Ts...>::operator=(variant<Ts...> &&rhs)
...
variant<Ts...> &variant<Ts...>::operator=(T &&t)
...
cnvrt_ctors<is_lvalue_arg>()[t_index]
...
cnvrt_assgnrs<is_lvalue_arg>()[m_index]
...
template<typename... Ts>
std::size_t util::variant<Ts...>::index() const
...
template<typename... Ts>
void variant<Ts...>::swap(variant<Ts...> &rhs)
...
typename util::type_list_element<Index, Types...>::type &get(util::variant<Types...> &v) {
    ...
    return const_cast<ReturnType &>(
            get<Index, Types...>(static_cast<const util::variant < Types... > & > (v)));
}
...
util::variant<Types...>::template index_of<T>();
...
namespace detail {
    ...
    typename std::enable_if<
            std::is_base_of < static_visitor < ReturnType, typename std::decay<Visitor>::type>,
    typename std::decay<Visitor>::type>::value, ReturnType>

    ::type
    invoke_class_visitor(Visitor &visitor, Value &&v, VisitorArgs &&...args) {
        return static_cast<static_visitor<ReturnType, typename std::decay<Visitor>::type> &>(visitor).operator()(
                CurIndex, std::forward<Value>(v), std::forward<VisitorArgs>(args)...);
    }

    //Visual Studio 2014 compilation fix: cast visitor to base class before invoke operator()
    template<std::size_t CurIndex, typename ReturnType, typename Visitor, class Value, typename... VisitorArgs>
    typename std::enable_if<std::is_base_of < static_indexed_visitor <
                            ReturnType, typename std::decay<Visitor>::type>,
    typename std::decay<Visitor>::type>::value, ReturnType>

    ::type
    invoke_class_visitor(Visitor &visitor, Value &&v, VisitorArgs &&...args) {
        return static_cast<static_indexed_visitor<ReturnType, typename std::decay<Visitor>::type> &>(visitor).operator()(
                CurIndex, std::forward<Value>(v), std::forward<VisitorArgs>(args)...);
    }
    ...
    apply_visitor_impl(Visitor &&visitor, Variant &&v, std::false_type not_processed,
        ...
        suppress_unused_warning(not_processed);
        if (v.index() == CurIndex) {
            return invoke_class_visitor < CurIndex, ReturnType >(visitor, get<CurIndex>(
                    v), std::forward<VisitorArgs>(args)... );
        }
...
auto visit(Visitor &visitor, const Variant &var, VisitorArg &&...args) ->

decltype(visitor(get<0>(var))
)
{
constexpr std::size_t
varsize = util::variant_size<Variant>::value;
static_assert(varsize
!= 0, "utils::variant must contains one type at least ");
...
return
detail::apply_visitor_impl<ReturnType, 0, varsize, Visitor>(
        std::forward<Visitor>(visitor),
...
auto visit(Visitor &&visitor, const Variant &var) ->
