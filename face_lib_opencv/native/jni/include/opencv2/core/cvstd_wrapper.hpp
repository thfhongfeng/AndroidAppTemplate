...
// C++ types should use regular "delete" operator.
    ...
    namespace sfinae {
        ...
        private:
            ...
            has_parenthesis_operator_check(typename std::is_same<typename std::decay<decltype(
                    std::declval<T>().operator()(std::declval<Args>()...))>::type, Ret>::type *);

            template<typename> static CV_CONSTEXPR std::false_type
            has_parenthesis_operator_check(...);
