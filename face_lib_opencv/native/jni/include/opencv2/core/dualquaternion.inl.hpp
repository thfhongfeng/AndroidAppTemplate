...
namespace cv {
    ...
    template<typename T>
    DualQuat<T>::DualQuat(const T vw, const T vx, const T vy, const T vz, const T _w, const T _x,
                          const T _y, const T _z):
    ...
    template<typename T>
    DualQuat <T> DualQuat<T>::createFromAngleAxisTrans(const T angle, const Vec<T, 3> &axis,
                                                       const Vec<T, 3> &trans) {
    ...
    template<typename T>
    DualQuat <T> DualQuat<T>::createFromPitch(const T angle, const T d, const Vec<T, 3> &axis,
    ...
    inline T DualQuat<T>::dot(DualQuat <T> q) const {
        return q.w * w + q.x * x + q.y * y + q.z * z + q.w_ * w_ + q.x_ * x_ + q.y_ * y_ +
               q.z_ * z_;
    }

    template<typename T>
    inline DualQuat <T>
    inv(const DualQuat <T> &dq, QuatAssumeType assumeUnit = QUAT_ASSUME_NOT_UNIT) {
    ...
    inline DualQuat <T> DualQuat<T>::operator-(const DualQuat <T> &q) const {
        return DualQuat<T>(w - q.w, x - q.x, y - q.y, z - q.z, w_ - q.w_, x_ - q.x_, y_ - q.y_,
                           z_ - q.z_);
    }
    ...
    template<typename T>
    inline DualQuat <T> operator*(const T a, const DualQuat <T> &q) {
        return DualQuat<T>(q.w * a, q.x * a, q.y * a, q.z * a, q.w_ * a, q.x_ * a, q.y_ * a,
                           q.z_ * a);
    }
    ...
    namespace detail {
        ...
        Matx<_Tp, 4, 4> jacob_exp(const Quat <_Tp> &q) {
            ...
            _Tp sinc_nv = abs(nv) < cv::DualQuat<_Tp>::CV_DUAL_QUAT_EPS ? _Tp(1.0) -
                                                                          nv * nv * _Tp(1.0 / 6.0) :
                          std::sin(nv) / nv;
            _Tp csiii_nv = abs(nv) < cv::DualQuat<_Tp>::CV_DUAL_QUAT_EPS ? -_Tp(1.0 / 3.0) :
                           (std::cos(nv) - sinc_nv) / nv / nv;
            Matx<_Tp, 4, 4> J_exp_quat{
                    std::cos(nv), -sinc_nv * q.x, -sinc_nv * q.y, -sinc_nv * q.z,
                    sinc_nv * q.x, csiii_nv * q.x * q.x + sinc_nv, csiii_nv * q.x * q.y,
                    csiii_nv * q.x * q.z,
                    sinc_nv * q.y, csiii_nv * q.y * q.x, csiii_nv * q.y * q.y + sinc_nv,
                    csiii_nv * q.y * q.z,
                    sinc_nv * q.z, csiii_nv * q.z * q.x, csiii_nv * q.z * q.y,
    ...
    template<typename T>
    inline DualQuat <T>
    power(const DualQuat <T> &dq, const T t, QuatAssumeType assumeUnit = QUAT_ASSUME_NOT_UNIT) {
    ...
    DualQuat <T> DualQuat<T>::sclerp(const DualQuat <T> &q0, const DualQuat <T> &q1, const T t,
                                     bool directChange, QuatAssumeType assumeUnit) {
    ...
    template<typename T>
    DualQuat <T> DualQuat<T>::dqblend(const DualQuat <T> &q1, const DualQuat <T> &q2, const T t,
    ...
    template<typename T>
    DualQuat <T>
    DualQuat<T>::gdqblend(InputArray _dualquat, InputArray _weight, QuatAssumeType assumeUnit) {
    ...
    template<typename T>
    template<int cn>
    DualQuat <T> DualQuat<T>::gdqblend(const Vec <DualQuat<T>, cn> &_dualquat, InputArray _weight,
                                       QuatAssumeType assumeUnit) {
