 ...
 *
 * @attr R.styleable#TransformativeImageView_base_maxScale
 * @attr R.styleable#TransformativeImageView_base_minScale
 * @attr R.styleable#TransformImageView_base_enableImageScale
 * @attr R.styleable#TransformImageView_base_enableImageTranslate
 * @attr R.styleable#TransformImageView_base_enableImageRotate
 * @attr R.styleable#TransformativeImageView_base_revertDuration
 * @attr R.styleable#TransformativeImageView_base_revert
 * @attr R.styleable#TransformativeImageView_base_scaleCenter
 */

public class TransformImageView extends AppCompatImageView implements IGestureView {
    ...
    private static final float MIN_SCALE_FACTOR = 1.0f; // 最小缩放比例
    ...
    private boolean mEnableImageScale = true; // 是否开启缩放功能
    private boolean mEnableImageTranslate = true; // 是否开启平移功能
    private boolean mEnableImageRotate = true; // 是否开启旋转功能
    private float mUserMaxScaleFactor = UNSPECIFIED_SCALE_FACTOR; // 用户指定的最大缩放比例
    private float mUserMinScaleFactor = 0.3f; // 用户指定的最小缩放比例
    ...
    private void obtainAttrs(AttributeSet attrs) {
        ...
        TypedArray typedArray = getContext()
                .obtainStyledAttributes(attrs, R.styleable.BaseTransformImageView);
        mUserMaxScaleFactor = typedArray.getFloat(
                R.styleable.BaseTransformImageView_base_maxScale, MAX_SCALE_FACTOR);
        mUserMinScaleFactor = typedArray.getFloat(
                R.styleable.BaseTransformImageView_base_minScale, UNSPECIFIED_SCALE_FACTOR);
        mEnableImageScale = typedArray.getBoolean(
                R.styleable.BaseTransformImageView_base_enableImageScale, true);
        mEnableImageTranslate = typedArray.getBoolean(
                R.styleable.BaseTransformImageView_base_enableImageTranslate, true);
        mEnableImageRotate = typedArray.getBoolean(
                R.styleable.BaseTransformImageView_base_enableImageRotate, true);
        mRevertDuration = typedArray.getInteger(
                R.styleable.BaseTransformImageView_base_revertDuration, DEFAULT_REVERT_DURATION);
        mOpenScaleRevert = typedArray.getBoolean(
                R.styleable.BaseTransformImageView_base_openScaleRevert, false);
        mOpenRotateRevert = typedArray.getBoolean(
                R.styleable.BaseTransformImageView_base_openRotateRevert, false);
        mOpenTranslateRevert = typedArray.getBoolean(
                R.styleable.BaseTransformImageView_base_openTranslateRevert, false);
        mOpenAnimator = typedArray.getBoolean(
                R.styleable.BaseTransformImageView_base_openAnimator, true);
        mScaleBy = typedArray.getInt(
                R.styleable.BaseTransformImageView_base_scaleCenter, SCALE_BY_IMAGE_CENTER);
        typedArray.recycle();
    }
...
//        setLayerType(View.LAYER_TYPE_SOFTWARE, null);
    ...
    private void initImgPositionAndSize() {
        ...
        // 计算缩放比例，使图片适应控件大小
        float containerHorizontalMinScaleFactor = Math.min(getWidth() / mImageRect.width(),
                getHeight() / mImageRect.height());
        float containerVerticalMinScaleFactor = Math.min(getHeight() / mImageRect.width(),
                getWidth() / mImageRect.height());

        float containerScaleFactor = containerHorizontalMinScaleFactor;

        // 初始图片缩放比例调整
        float scaleFactor = containerScaleFactor * INIT_SCALE_FACTOR;
        ...
        if (mUserMinScaleFactor != UNSPECIFIED_SCALE_FACTOR) {
            mHorizontalMinScaleFactor = containerHorizontalMinScaleFactor * mUserMinScaleFactor;
            mVerticalMinScaleFactor = containerVerticalMinScaleFactor * mUserMinScaleFactor;
            mMinScaleFactor = mHorizontalMinScaleFactor;
        }
        if (mUserMaxScaleFactor != UNSPECIFIED_SCALE_FACTOR) {
            mMaxScaleFactor = containerScaleFactor * mUserMaxScaleFactor;
        }
    }
    ...
    private boolean mCanDrag = false; // 是否可以平移
    ...
    private boolean canScale() {
        return mEnableImageScale && mCanScale;
    }

    private boolean canDrag() {
        return mEnableImageTranslate && mCanDrag;
    }

    private boolean canRotate() {
        return mEnableImageRotate && mCanRotate;
    }

    @Override
    public boolean onTouchEvent(MotionEvent event) {
        //双击的时候接受触摸事件 并且不需要去移动
    ...
    private boolean scale(MotionEvent event) {
        ...
        float nextScaleFactor = mScaleFactor * scaleFactor;
        // 获取图片当前是水平还是垂直
        ...
        if (imgOrientation == HORIZONTAL
                && nextScaleFactor < mHorizontalMinScaleFactor ||
                imgOrientation == VERTICAL
                        && nextScaleFactor < mVerticalMinScaleFactor ||
                nextScaleFactor > mMaxScaleFactor) {
            return false;
        }

        // 更新当前图片的缩放比例
        mScaleFactor = nextScaleFactor;
    ...
    @Override
    public boolean canScroll(boolean checkV, int dx, int x, int y) {
        if (mEnableImageTranslate) {
            if (mImageRect.width() > getWidth()) {
                if (dx > 0 && mImageRect.left < 0) {/*图片左边界与控件之间没有间隙*/
                    return true;
                } else if (dx < 0 && mImageRect.right > getWidth()) {/*图片右边界与控件之间没有空隙*/
                    return true;
                }
            }
        }
    ...
    public void setMaxScaleFactor(float maxScaleFactor) {
        mUserMaxScaleFactor = maxScaleFactor;
    }
    ...
    public void enableImageImageScale(boolean enable) {
