...
import com.pine.template.base.model.VersionModel;
...
public class ApkVersionManager {
    ...
    private void startUpdate(final Activity activity,
        ...
        String fileName = versionEntity.getAppKey() + "_" + versionEntity.getVersionName() + ".apk";
        File oldFile = new File(mDownloadDir, fileName);
        if (oldFile.exists() && versionEntity.getApkSize() > 0 && oldFile.length() == versionEntity.getApkSize()) {
