package com.pine.template.base.model;
        ...
        return new JsonCallback() {
            ...
            public void onResponse(int what, JSONObject jsonObject, Response response) {
                if (REQUEST_QUERY_VERSION_INFO == what) {
                    if (jsonObject.optInt(BaseConstants.CODE) == 0) {
                        T retData = new Gson().fromJson(jsonObject.toString(), new TypeToken<VersionEntity>() {
                        }.getType());
                        if (callback != null) {
                        ...
                        if (callback != null) {
                            callback.onFail(new MessageException(jsonObject.optString("Msg")));
                        }
                    }
                }
            }
