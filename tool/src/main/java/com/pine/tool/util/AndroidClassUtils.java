...
public class AndroidClassUtils {
     ...
     * @return 所有class的集合
     */
    public static List<String> getFileNameByPackageName(Context context, final String packageName)
            ...
            getThreadPoolExecutoInstance().execute(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        LogUtils.e(TAG, "Scan map file in dex files made error.", ignore);
                    } finally {
                        if (null != dexfile) {
                            try {
                                dexfile.close();
        ...
        LogUtils.d(TAG, "Filter " + classNames.size() + " classes by packageName <" + packageName + ">");
        return classNames;
    }
...
//        通过是否存在sp中的multidex.version是不准确的，因为从低版本升级上来的用户，是包含这个sp配置的
        ...
        if (AppUtils.isApkDebuggable(context)) { // Search instant run support only debuggable
    ...
    private static List<String> tryLoadInstantRunDexFile(ApplicationInfo applicationInfo) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && null != applicationInfo.splitSourceDirs) {
            ...
            LogUtils.d(TAG, "Found InstantRun support");
        } else {
            try {
