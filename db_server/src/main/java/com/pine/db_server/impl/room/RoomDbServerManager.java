...
import com.pine.db_server.impl.room.repository.AccountRepository;
...
public class RoomDbServerManager implements IDbServerManager {

    private static volatile RoomDbServerManager mInstance;
    private static volatile HashMap<String, DbSession> mSessionMap = new HashMap<>();

    private RoomDbServerManager(@NonNull Context context) {
        new SQLiteDbHelper(context).getReadableDatabase();
    }

    public synchronized static RoomDbServerManager getInstance() {
        if (mInstance == null) {
            mInstance = new RoomDbServerManager(AppUtils.getApplicationContext());
        }
        return mInstance;
    }
    ...
    public synchronized DbResponse callCommand(@NonNull Context context, @NonNull DbRequestBean requestBean,
        ...
        if (DbUrlConstants.Query_BundleSwitcher_Data.equals(requestBean.getUrl())) {
            ...
            return AccountRepository.getInstance(context).register(requestBean, header);
